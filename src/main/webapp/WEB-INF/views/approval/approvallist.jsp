...
<div >
	...
	<button type="button" class="btn-sm btn-secondary" id="rejectList">반려</button>
        ...
        <form action="">
            <h2 style="margin-bottom: 0; margin-top:70px">전체 발신목록</h2>
                ...
                <c:if test="${applist != null}">
	                <c:forEach var="list" items="${applist}">
		                ...
		                <tr>
		                    ...
		                    <td>${list['M_NAME']}</td>
		                    <td>${list['AF_NAME']}</td>
		                   <!--  <td>${list['APPROVAL_TITLE']}</td> --><!-- 상세보기 페이지로 이동 - 내가 요청한 결재 진행 상황 확인 -->
		                    <td><a href="${path}/Approval/check.kosmo?approval_code=${list['APPROVAL_CODE']}" >${list['APPROVAL_TITLE']}</a></td>
